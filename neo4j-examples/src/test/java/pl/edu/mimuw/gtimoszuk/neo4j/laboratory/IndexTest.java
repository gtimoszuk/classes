...
public class IndexTest extends BaseTest {
	...
	@Test
	public void basicTimelineTest() throws InterruptedException {
		Transaction tx = graphDb.beginTx();
		try {
			Node rootNode = graphDb.createNode();
			LOG.info("root node id: " + rootNode.getId());
			...
			// GTI: not really but life...
			Iterable<Node> nodes = timeline.getAllNodesBetween(startTime, System.currentTimeMillis() - 3000);
			Iterator<Node> iterator = nodes.iterator();
			int i = 0;
			while (iterator.hasNext()) {
				LOG.info("number: " + i + " node id: " + iterator.next().getId());
				++i;
			}
		} finally {
			tx.finish();
