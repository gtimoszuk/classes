...
public class CreateJavaSchema {
	...
	private static String[] allAttrs = { "javaSerializedObject", "javaFactoryLocation", "javaReferenceAddress",
	        "javaFactory", "javaClassName", "javaClassNames", "javaDoc", "javaSerializedData", "javaCodebase",
	        "javaFactory", "javaReferenceAddress" };
	 ...
	 * Updates the schema:
	 * 
	 * Delete obsolete attributes: javaSerializedObject javaFactoryLocation javaReferenceAddress javaFactory
	 * javaClassName + all the new ones that we're going to add Add new and updated attributes: javaSerializedData
	 * javaCodebase javaClassName javaClassNames javaFactory javaReferenceAddress javaDoc
	 * 
	 * Delete obsolete object classes: javaNamingReference javaObject + all the new ones that we're going to add Add new
	 * and updated object classes: javaObject javaSerializedObject javaMarshalledObject javaNamingReference
	 */
	 ...
	 * 
	 * Modification of the (RFC 2252) schema descriptions is not supported in Active Directory. Instead, the Active
	 * Directory (internal) schema must be modified.
	 */
	 ...
	 * 
	 * This method maps the LDAP schema definitions in RFC 2713 onto the proprietary attributes required by the Active
	 * Directory schema.
	 * 
	...
	protected void insertADAttributes(DirContext rootCtx, DirContext schemaCtx) throws NamingException {
		...
		attrs1.put(new BasicAttribute("description", "Fully qualified name of distinguished Java class or interface"));
	 ...
	 * Directory schema.
	 * 
	 * The resulting object class definitions differ from those of RFC 2713 in the following ways:
	 * 
	 * - Abstract and auxiliary classes are now defined as structural. - The javaObject class now inherits from
	 * javaContainer. - The javaNamingReference, javaSerializedObject and javaMarshalledObject now inherit from
	 * javaObject.
	 * 
	 * The effect of these differences is that Java objects cannot be mixed-in with other directory entries, they may
	 * only be stored as stand-alone entries.
	 * 
	 * The reason for these differences is due to the way auxiliary classes are supported the Active Directory. Only the
	 * names of structural classes (not auxiliary) may appear in the object class attribute of an entry. Therefore, the
	 * abstract and auxiliary classes in the Java schema definition are re-defined as structural.
	 */
	...
	protected void flushADSchemaMods(DirContext rootCtx) throws NamingException {
		...
		rootCtx.modifyAttributes("", new ModificationItem[] { new ModificationItem(DirContext.ADD_ATTRIBUTE,
	...
	protected void printUsageAux(String msg, String key) {
		...
		System.out.println("\t\t -sn   use workaround for Netscape Directory pre-4.1 schema bug");
		System.out.println("\t\t -sn41 use workaround for Netscape Directory 4.1 schema bug");
